...
sn_LAA_FB<- readRDS("~/Desktop/Human_LA_LAA/RDS/AurN_cluster_sn_LAA_fibroblastmyofibroblast2.rds")
...
new.cluster.ids <- c("FB.0","FB.1","FB.2","FB.3","FB.4")
names(new.cluster.ids) <- levels(sn_LAA_FB)
sn_LAA_FB<- RenameIdents(sn_LAA_FB, new.cluster.ids)



#非免疫細胞全体+Myeloid

sc_sn_LAA <- merge(sn_LAA_FB, y = sc_LAA_imuune)



levels(sc_sn_LAA)

new.cluster.ids <- c("FB.0","FB.1","FB.2","FB.3","FB.4","My.0","My.1","My.2","My.3","My.4","My.5","My.6","My.7","My.8","My.9","My.10","My.11","My.12","My.13","My.14")
...
#sc_myeloid+sn_FB
saveRDS(cellchat, "~/Desktop/Human_LA_LAA/RDS/cellchat_result_Aur_scAll.Myeloid_sn.FB_merged_IL1new.rds")

cellchat <- readRDS("~/Desktop/Human_LA_LAA/RDS/cellchat_result_Aur_scAll.Myeloid_sn.FB_merged_IL1new.rds")
...
pathways.show <- c("VEGF")  # 上記で選んだpathwayを記入
...
par(mfrow=c(1,1))
pdf(file ="~/Desktop/Human_LA_LAA/Results/cellchat_sc_sn/myeloid+FB.pdf", width = 20, height =16) # RStudioのキャンパスサイズだと、plotが小さくなるのでpdfに描画する
...
#### ligand-receptor interaction databaseを読み込む####
